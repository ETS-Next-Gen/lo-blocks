...
/**
 * Select locale tiers based on curation status.
 *
 * Classifies available locales into:
 * - curated: All content human-authored (autogenerated: false or undefined)
 * - bestEffort: Any content auto-generated (autogenerated: true)
 *
 * Memoized to prevent unnecessary re-renders.
 */
export interface LocaleTiers {
  curated: string[];
  bestEffort: string[];
  all: string[];  // All locales, curated first, then best-effort
}

/**
 * Select locale tiers from Redux state.
 * Returns memoized result - use in useSelector or wrap with useMemo in components.
 */
export function selectLocalesTiers(state: RootState): LocaleTiers {
  const olxjson = state.application_state?.olxjson;
  ...
  // Track locales by whether they have ONLY native content or ANY autogenerated content
  const localeStatus: Record<string, 'curated' | 'mixed'> = {};
  ...
  for (const sourceState of Object.values(olxjson)) {
    ...
    // where blockEntry = { olxJson: { locale: OlxJson, ... }, ... }
    for (const blockEntry of Object.values(sourceState)) {
      if (blockEntry?.olxJson && typeof blockEntry.olxJson === 'object') {
        const langMap = blockEntry.olxJson as Record<string, any>;

        for (const [locale, olxJson] of Object.entries(langMap)) {
          // Check if this block in this locale is autogenerated
          const isAutogenerated = (olxJson as any)?.autogenerated === true;

          if (isAutogenerated) {
            // If ANY block is autogenerated, mark locale as mixed/bestEffort
            localeStatus[locale] = 'mixed';
          } else if (localeStatus[locale] !== 'mixed') {
            // Only mark as curated if we haven't already seen autogenerated content
            localeStatus[locale] = 'curated';
          }
        }
      }
    }
  ...
  const curated = Object.entries(localeStatus)
    .filter(([_, status]) => status === 'curated')
    .map(([locale, _]) => locale)
    .sort();

  const bestEffort = Object.entries(localeStatus)
