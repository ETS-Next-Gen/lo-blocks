 ...
 * - curated: Native/official languages (no autogenerated flag)
 * - bestEffort: Autotranslated languages (autogenerated: true)
 *
...
export function selectLocalesTiers(state: RootState): LocaleTiers {
  ...
  const bestEffortSet = new Set<string>();
    ...
    for (const blockEntry of Object.values(sourceState)) {
      if (blockEntry?.olxJson && typeof blockEntry.olxJson === 'object') {
        ...
        for (const [locale, olxJson] of Object.entries(langMap)) {
          // Check if any block in this locale has autogenerated flag
          const isAutogenerated = (olxJson as any)?.autogenerated === true;

          if (isAutogenerated) {
            bestEffortSet.add(locale);
          } else {
            curatedSet.add(locale);
          }
        }
      }
    }
  ...
  const curated = Array.from(curatedSet).sort();
  const bestEffort = Array.from(bestEffortSet).sort();
