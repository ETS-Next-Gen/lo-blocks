...
/** Parse YAML frontmatter from OLX content. Returns {} if none found. */
...
async function doTranslation(
  ...
  const initialMeta = parseFrontmatter(sourceContent);
  if (initialMeta.autogenerated && initialMeta.source_file) {
    const originalFileName = initialMeta.source_file;
  ...
  // 3. Parse source metadata and prepare LLM call
  ...
  const descriptionLine = sourceMeta.description
...
${descriptionLine ? '- If a DESCRIPTION: line is provided, translate it and output it on its own line BEFORE the OLX, in the format: DESCRIPTION: <translated text>' : ''}`;
  ...
  let translatedDescription = sourceMeta.description; // fallback to source
