    ...
    reducer: (files, outputFile, gitStatusMap) => {
      ...
      for (const filePath of files) {
        ...
        if (assets.blockGitStatus) {
          annotations += `${baseName}.gitStatus = '${assets.blockGitStatus}';\n`;
        }
        if (assets.docs) {
          annotations += `${baseName}.readme = '${assets.docs}';\n`;
          if (assets.docsGitStatus) {
            annotations += `${baseName}.readmeGitStatus = '${assets.docsGitStatus}';\n`;
          }
        }
...
export function generateAllRegistryContents() {
  ...
  for (const [name, config] of Object.entries(registryTypes)) {
    ...
    result[name] = {
      content: config.fileHeader + config.reducer(files, config.outputFile, gitStatusMap),
