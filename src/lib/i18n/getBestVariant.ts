...
export function extractLocalizedVariant<T>(
  ...
  // Fall back: prefer human-authored (non-autogenerated) content over translations.
  // Uses duck-typing so this works for OlxJson variant maps (which have autogenerated
  // flag) and is a no-op for simpler T types like strings.
  const preferred = availableVariants.find(v => {
    const val = variantMap[v] as any;
    return val && typeof val === 'object' && !val.autogenerated;
  });
