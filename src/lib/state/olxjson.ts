...
export function selectVariantTiers(state: RootState): VariantTiers {
  ...
  // Track variants by whether they have ONLY native content or ANY autogenerated content
  ...
  for (const sourceState of Object.values(olxjson)) {
    ...
    for (const blockEntry of Object.values(sourceState)) {
      if (blockEntry?.olxJson && typeof blockEntry.olxJson === 'object') {
        const variantMap = blockEntry.olxJson as Record<string, any>;

        for (const [variant, olxJson] of Object.entries(variantMap)) {
          // Check if this block in this variant is autogenerated
          const isAutogenerated = (olxJson as any)?.autogenerated === true;

          if (isAutogenerated) {
            // If ANY block is autogenerated, mark variant as mixed/bestEffort
