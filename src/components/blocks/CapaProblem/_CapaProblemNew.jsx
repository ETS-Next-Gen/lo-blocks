// src/components/blocks/CapaProblem/_CapaProblemNew.jsx
'use client';
...
export default function _CapaProblemNew(props) {
  const { id, kids = [], locals } = props;

  // First render problem content to populate dynamic OLX DOM (renderedKids)
  const content = renderCompiledKids({ ...props, kids });

  // Use locals for educational logic instead of inline inferRelatedNodes
  const graderIds = locals?.graders ? locals.graders(props) : [];

  console.log('_CapaProblemNew: graderIds from locals:', graderIds);
